...
public class BookController {
    ...
    @GetMapping("/findById")
    public Result findById(@RequestParam(required = false) Long id) {
        if (id == null || id <= 0) {
            return Result.error("请输入有效的图书编号");
        }
        Book book = bookService.getById(id);
        if (book == null) {
            return Result.error("未找到该图书");
        }
        return Result.success(java.util.List.of(book));
    }
